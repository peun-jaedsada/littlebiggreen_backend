                ...
                @forelse ($datas as $key=> $item)
                    <tr class="">
                        ...
                        <td>
                            ...
                            @include('backend._module.modaldelete',['modal'=>'deleteModal'.$item->id,'modal_id'=>$item->id,'modal_title'=>$item->blog_title ,'url_delete'=>route('blog.destroy',$item->id)])
                            {{-- <a href="javascript:void(0)" class="btn btn-sm btn-danger waves-effect waves-light" onclick="deleteModal({{$item->id}})"><i class="fe-trash"></i></a> --}}
                            {{-- @include('backend._module.modaldelete',['modal_id'=>$item->id,'modal_title'=>$item->blog_title ,'url_delete'=>route('blog.destroy',$item->id)]) --}}
                        </td>
                    </tr>
                @empty
                    <tr class="">
